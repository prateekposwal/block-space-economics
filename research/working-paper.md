...
**The headline is a range, not a point.** The replication factor N (nodes bearing storage cost) is **not directly observable**. We have two different measured quantities: a **measured reachable-node count of 26,586** (btcnodes scan, 2026-09-16) and an **address-manager sample of ≥32,000 gossiped addresses** (Bitcoin Core `getnodeaddresses 32000`, 2026-08-02 — a lower bound on *addresses*, **not** a node count). Independent reachable estimates span ~10K–100K. Because SCCR is exactly inverse-linear in N, the result is stated in four lines (exact figures appear in the table below and §5.1):

> **Re-based 2026-09-17 (v2.1.1).** N was 32,000 — an address-manager *sample* at the request ceiling, never a node count. It is now the **measured reachable-node count (validating; a floor) of 26,586** (btcnodes crawl, 2026-09-16). Because SCCR is inverse-linear in N, every SCCR figure in this paper is multiplied by **32,000/26,586 = 1.2036x (+20.4%)**. The value 32,000 is not retained; where older figures appear below they carry this factor. N remains a **lower bound** (non-listening nodes are unobservable), so the true SCCR is if anything lower.
...
All quantities, units, and formulas live in **`research/model-spec.json` (v2.1.1)** — a single canonical source that every script imports. No script redefines a model constant.

| Quantity | Symbol | Units | Value | Source |
|---|---|---|---|---|
| Annual node cost | C | USD/yr | 925 | `utxo_cost_model` component sum (924.35) rounded |
| Replication factor | N | nodes | 32,000 *(provisional)* | **Not directly observable.** Retained for reproducibility. 32,000 is the address-manager *address* sample (≥32K gossiped addresses), **not a node count**; measured reachable nodes = 26,586. Re-based in Phase C — see `/research/verification-population` |
...
**Live re-measure (canonical, 2026-08-02):** the ratio moves with the fee market. The canonical re-measure recorded in `research/model-spec.json` v2.0.1 is **0.2252** (168 blocks, 2026-08-02); the 0.2228 above is the same capture window re-read at the time of writing (167 blocks). The 0.293 figure is the dated 2026-08-01 snapshot. The single source of truth is `research/model-spec.json` (v2.1.1, canonical); all surfaces must read the live value from `node tools/research/storage-ratio.js`, never a hardcoded figure. The v2.0.0 N=60K-era values (0.1719 / 0.1535) are superseded.
...
> **Pre-re-base record (N=32,000).** This section preserves the original derivation, which was written against the then-canonical N=32,000 (an address-manager sample). The canonical N is now **26,586** (see the re-base note at the top of the paper): multiply every SCCR value in this section by **1.2036** to place it on the current basis. The *reasoning* — that N dominates and that the strong "100% below 1×" form does not survive a favourable N — is unchanged.
...
**Live recompute (2026-08-02 baseline, SCCR = 0.2228 at N=32K):** because the baseline ratio itself rose, the average now inverts at **N ≈ 7,130 nodes** or **BTC ≈ $283,000**; the "100% below 1×" break on the dated capture is unchanged at N ≈ 49,200 (the max block is 0.832× at N=32K on the live capture, still below 1×). Both the dated and live thresholds are reported; the model-spec v2.1.1 note retains the v2.0.0-era values (10.3K / $366K).

**The node count is not directly observable — and our earlier framing of it was wrong.** We previously described `getnodeaddresses` returning 32,000 as a *lower bound on reachable nodes*. It is not. `getnodeaddresses 32000` returns addresses from the node's **address manager** — the set it has learned via **addr gossip** — and 32,000 was the *requested ceiling*, returned in full. It is therefore a lower bound on **gossiped addresses**: a set that includes stale and unreachable entries and whose size depends on the node's peer count and uptime (this node: 8 outbound connections). It is **not** a node count and **not** a bound on reachable nodes. Two quantities are now reported separately: a **measured reachable-node count of 26,586** (btcnodes crawl, 2026-09-16) and the **address-manager sample** (≥32,000 gossiped addresses, 2026-08-02). The total population — including non-listening/private nodes — remains unobservable; independent reachable estimates span ~10K–100K. See `/research/verification-population`.

**Consequence of the provisional N=32,000 (an address sample, not a node count; recomputed, dated 156-block capture):**

| Metric | At N=60K (old assumption) | At N=32K (provisional — address sample, not a node count) |
|---|---|---|
...
**This is a substantive finding, not a cosmetic one.** With a defensible node count, fees cover *more* of the modeled storage cost than the 60K assumption implied (~29% vs ~17%), and a handful of high-fee blocks now *exceed* 1× coverage. The direction of the headline is unchanged — most blocks are still below 1× — but the strong form ("100% below 1×") does **not** survive the provisional N=32K (an address sample, not a node count) on the dated capture. **The defensible claim is: ~22–29% average coverage at the provisional N=32K, ~98.7–100% of sampled blocks below 1×, and ~0.07–0.71 average across the true-N band (10K–100K) — with the caveat that N itself is unobservable and 32K is an address sample, not a node count.** (1× remains a descriptive calibration point, not a normative target — §1.)
...
**Current-N-band Monte Carlo** (`research/sccr_monte_carlo_range.py`, 10,000 draws, N ~ Tri(10K, 100K, mode 26,586) — the paper's stated uncertainty band with the measured reachable count as mode; C ~ Tri($600, $1400, mode $925), T ~ Tri(5, 15, mode 10), P ~ Tri($30K, $120K, mode $63,018); anchored at the live baseline and cross-checked against the frozen capture):
...
| `bw_cost_per_year_net` | **~$126K/yr** | × N=32K (provisional) |
...
**First-party instruments aimed at the unobservable set.** A listening node counts the peers that **dial in** (`inbound_census.py`). This is often read as direct evidence of non-listening nodes, but that over-reads it: a **listening** node can dial us too, so the inbound set is `{private diallers} ∪ {public diallers}` — an **upper bound** on private participation, not a count of non-listening nodes. Splitting the two would require dialling each peer back. What the inbound set *does* give for free is the peer's **service bits** (`NODE_NETWORK` vs `NODE_NETWORK_LIMITED`), which sample the replication factor of a population no crawler can reach. Sampling the address manager over time (`addrman_churn.py`) measures address **persistence**, turning "stale IP" from an assertion into a rate.
...
**Why the conclusion survived the correction.** Although the correction increased the estimated SCCR by ~10× on a same-capture basis (0.0172 → 0.1719 (dimensionless) at the then-assumed N=60K) — and by ~11.5× across the as-reported headlines (v1.0.0 **0.0149** → v2.0.0 **0.1719**, a change that also spans different capture windows) — every sampled block still remained below the modeled full-cost threshold (1×) under the then-assumed node count (N=60K). At the provisional N=32K the average rose further to **~0.225 (22.5%)** on the 2026-08-02 live re-measure (168 blocks), with ~99% of sampled blocks still below the 1× threshold. The magnitude of the measurement moved by an order of magnitude; the *direction* of the finding did not.

*Note: the table above documents the 10× time-horizon correction at the then-assumed N=60K. (A 2026-09-17 correction also removed the '32K node lower bound' claim — see §5.4.) A separate, subsequent change replaced the 60K assumption with the address-manager sample (N=32K, provisional), which moves the average to ~0.22–0.29 and the below-1× share to ~99–100% (§5.4). A third correction, 2026-09-17, removed the claim that 32K is a *node-count lower bound*: it is an address sample, and the measured reachable-node count is 26,586. All three are documented.*
...
The correction increased the estimated SCCR by an order of magnitude but did not reverse the paper's qualitative conclusion. This distinction is important: the implementation error affected the estimated magnitude of the measurement, whereas the underlying hypothesis was evaluated against the corrected model and remained supported under the paper's assumptions. Every quantity in this paper is regenerated from `research/model-spec.json` (v2.1.1) by three independent implementations (JS, Python, standalone C); no script redefines a model constant, and the full capture log is retained in `captured-data/bsahi.db`.

**Independent reproduction (status).** The frozen capture and the three implementations agree (2026-09-16 snapshot: avg SCCR 0.2406 at the pre-re-base N=32,000 — **0.289645 re-based to N=26,586** — min 0.0490, max 1.0757 pre-re-base, 153/155 blocks below 1×; 152/155 re-based), and the one-command reproduction path has been verified from a clean clone of the public repository. This is internal consistency. The paper's reproducibility claim will be stated, once external runs land, as: *"Independently reproduced by external participants following the published reproduction protocol"* — not "externally verified." One independent success is good; three is excellent. An external run is the only outstanding submission gate (D5; `research/reproduce/external-reproduction.md`). *(Re-based 2026-09-16: the original 2026-08-02 0.2186/171-block capture was never committed and the GH pipeline had refreshed the kit input in place; live refreshes now go to `captured-data/sccr-live/`, the kit input is frozen.)*

## 7. Limitations and Future Work

**Limitations:**
1. **The node count is not directly observable.** 32,000 is an address-manager sample (≥32K **gossiped addresses**, not nodes); the measured reachable-node count is 26,586, and the non-listening population is invisible to remote scans. Independent reachable estimates span ~10K–100K (pruned vs. archival). The SCCR is inversely proportional to node count: at the live baseline the average inverts above 1× only below ~7.1K nodes, and the "100% below 1×" claim breaks below ~49K nodes at the dated capture (see §5.4).
...
2. **A better storage-cost model reverses the conclusion.** The denominator (`C·T·N / R_blocks`) rests on a homogeneous node-cost model, an assumed T=10 horizon, and a provisional N=32K that is an *address* sample, not a node count. If a defensible refinement — measured pruned-vs-archival retention, regional cost heterogeneity, or corrected discounting — moves the banded result (~22–29% coverage, most blocks below 1×) to a qualitatively different conclusion, the paper's central claim is falsified in its current form.
...
| 2 | Replication factor **N = 26,586** | **M** | A measured reachable-node count (a **lower bound** — non-listening nodes are unobservable); the earlier 32,000 was an addrman *address* sample, not a node count (§5.4); true set estimated 10K–100K; SCCR is inversely proportional to N | A defensible complete census showing the true reachable set differs materially from 32K in a direction that moves the ~0.07–0.71 band; the "~99–100% below 1×" claim already breaks at N ≈ 49K (§5.4) |
