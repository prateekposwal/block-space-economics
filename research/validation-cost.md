...
| Network-wide (N = 26,586) | **~$16K–160K/yr** | × the measured reachable node count (26,586; once the pre-re-base 32,000); central ~$32–64K/yr |
...
- Network-wide numbers scale linearly in N and inherit the node-count caveat
  (the addrman sample bounds known addresses, not nodes; the true node band is 10K–100K).

## 5. Falsifiable claim (v1)

> **"Validation cost per full node per year is < $100 — bounded from above by the entire node hardware+operating budget C = $925/yr (model-spec v2.1.1), with a central order-of-magnitude estimate of ~$1–2/yr per node."**
