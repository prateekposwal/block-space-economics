...
## v2.3.0 UPDATE (2026-09-15) — Reconstruction executed from frozen daily aggregates

The v2.2.0 "C (Broken)" verdict is superseded for the **fee/price/blocksize** legs.
The historical node-count leg remains a documented approximation (grade C/D), not a
repo measurement.

**What changed:** `tools/research/sccr_historical_reconstruct.py` now reads six frozen
daily series (blockchain.info charts: market-price, transaction-fees, n-transactions,
n-transactions-per-block, avg-block-size, hash-rate) from
`captured-data/historical/blockchain.info/` and computes per-era SCCR with the canonical
model (`L_net = C x T x N / R_blocks`, C=925, T=10, R=52,596). Deterministic, offline,
reproducible.

**Resulting era series (fee_USD/block, SCCR at both node-count scenarios):**

| Era | fee_USD/block | SCCR @ N=32K | SCCR @ era-N | Note |
|---|---|---|---|---|
| 2017 | $6,997 | 1.24 | 4.97 (N=8K) | era-adjusted ~5×, NOT Q7's 10× |
| 2019 | $2,693 | 0.48 | 1.53 (N=10K) | sub-1× at principal census |
| 2021 | $18,736 | 3.33 | 8.88 (N=12K) | era-adjusted matches Q7 (~8×) |
| 2023 | $12,904 | 2.29 | 4.89 (N=15K) | matches Q7 (~5×) |
| 2024 | $23,573 | 4.19 | 7.88 (N=17K) | era-adjusted HIGHER than Q7 (4.8) |
| 2026* | $1,391 | 0.25 | 0.25 (N=32K) | repo live captures authoritative |

**Q7 verification verdicts:** 2021 ✅, 2023 ✅ (within 50%). 2017 ❌ (recomputed ~5×,
not ~10×), 2024 ❌ (recomputed ~7.9×, not ~4.8×).

**What the era series teaches (2026-09-15 reading):**
- The 2017 "~10× SCCR" claim is overstated by ~2× even at era-adjusted nodes.
- 2024 (Runes launch + Ordinals fee pressure) is the highest absolute fee year,
  era-adjusted SCCR ≈ 7.9×, above the Q7 claim.
- 2025-2026 is unambiguously the first sustained sub-1× regime at the principal census —
  consistent with the working paper's "first sustained sub-1× era" claim.
- SCCR moves roughly 10× across eras (0.25 → ~8), driven by fee price volatility, not by
