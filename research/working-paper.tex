...
%  Working Paper v2.2.0 (model-spec.json v2.1.1) — LaTeX source
%
...
\date{Working Paper v2.2.0 \textbullet{} model-spec v2.1.1 \textbullet{} 2026-09-23}
...
\textbf{The headline is a range, not a point.} The replication factor $N$ (nodes bearing storage cost) is \textbf{not directly observable}. We report two measured quantities: a \textbf{measured reachable-node count (validating; a floor) of 26{,}586} (btcnodes crawl, 2026-09-16) and an \textbf{address-manager sample of $\geq$32{,}000 gossiped addresses} (Bitcoin Core \texttt{getnodeaddresses 32000}, 2026-08-02 --- a lower bound on \emph{addresses}, \textbf{not} a node count). Independent reachable estimates span $\sim$10K--100K. Because SCCR is exactly inverse-linear in $N$, the result is stated in four lines (exact figures appear in \S\ref{sec:5}):

\textbf{Re-based 2026-09-17 (model-spec v2.1.1).} $N$ was 32{,}000 --- an address-manager \emph{sample} at the request ceiling, never a node count. It is now the measured reachable-node count (validating; a floor) of \textbf{26{,}586}. Because SCCR is inverse-linear in $N$, every SCCR figure is multiplied by \textbf{32{,}000/26{,}586 = 1.2036$\times$ (+20.4\%)}. $N$ remains a \textbf{lower bound} (non-listening nodes are unobservable), so the true SCCR is if anything lower.

\begin{itemize}
\item \textbf{Dated baseline band (Aug 02--15): $\approx$0.26 coverage at $N=26{,}586$}, with $\sim$100\% of sampled blocks below $1\times$ storage cost.
\item \textbf{Live series (Aug 24): 0.4813 at $N=26{,}586$, 94.37\% of blocks below $1\times$}; the \textbf{Sep 07 reading is 0.307344 (138 blocks, 100\% below $1\times$)} --- the coverage ratio is \textbf{fee-market-driven and time-varying}, not a fixed point.
\item \textbf{Dynamic finding:} SCCR moved \textbf{0.1895 (Aug 16) $\rightarrow$ 0.5427 (Aug 21) $\rightarrow$ 0.3073 (Sep 07)} across a growing daily series (\textbf{2026-08-02 $\rightarrow$ 2026-09-18, 38 points}) measured live in \texttt{data/sccr\_history.json} --- dynamic measurement strengthens the paper.
\item \textbf{Model uncertainty: depends strongly on the replication factor $N$} --- the average spans $\sim$0.86 at $N=10$K to $\sim$0.09 at $N=100$K; this $N$-band range ($\sim$0.09--0.86) is a \emph{different} uncertainty from the dated/live observations above and carries the dominant risk.
\end{itemize}
...
All quantities, units, and formulas live in \texttt{research/model-spec.json} (v2.1.1) --- a single canonical source that every script imports. No script redefines a model constant.
\begin{center}
...
Replication factor & $N$ & nodes & 32{,}000 \emph{(pre-re-base)} & \textbf{Not directly observable.} Retained for reproducibility. 32{,}000 is the address-manager \emph{address} sample ($\geq$32K gossiped addresses), \textbf{not} a node count; measured reachable-node count = 26{,}586 (validating; a floor). Re-based in Phase C --- see \texttt{/research/verification-population} \\
...
Measured from live \texttt{fee\_history} captures. \textbf{The current canonical reading is SCCR 0.4307} (2026-09-23, 137 blocks, $N$=26{,}586, model-spec v2.1.1). The figures below are the \textbf{pre-re-base record at $N=32{,}000$}, retained for reproducibility; because SCCR is inverse-linear in $N$, multiply any of them by \textbf{32{,}000/26{,}586 = 1.2036} for the current basis (see the re-base note above). $N$ is not directly observable: 32{,}000 was the address-manager sample ($\geq$32K gossiped addresses, \textbf{not} a node count) and the measured reachable-node count is 26{,}586 (\S\ref{sec:5.4}). The headline, in four lines (exact numbers in the tables below):

\begin{itemize}
\item \textbf{Dated baseline band (Aug 02--15): $\approx$0.22} at $N=32$K (\textbf{$\approx$0.27 at $N=26{,}586$}), with $\sim$100\% of sampled blocks below $1\times$ storage cost.
\item \textbf{Live series (Aug 24): 0.4813} at $N=26{,}586$, \textbf{94.37\% of blocks below $1\times$} (true \textbf{Sep 07 = 0.307344}, 100\% below) --- the coverage ratio is fee-market-driven and time-varying.
\item \textbf{Dynamic finding:} SCCR moved \textbf{0.16 $\rightarrow$ 0.45 $\rightarrow$ 0.40} across a 22-point daily series (2026-08-02 $\rightarrow$ 2026-09-07).
\item \textbf{Model uncertainty: depends strongly on the replication factor $N$} --- the true-$N$ band (\textbf{$\sim$0.07--0.71}, \S\ref{sec:5.4}) is a \emph{different} uncertainty from the observed-sample band above.
\end{itemize}
...
\end{tabular}
\end{center}
The ratio moves with the fee market; the single source of truth is \texttt{research/model-spec.json} (v2.1.1, canonical); all surfaces must read the live value from \texttt{node tools/research/storage-ratio.js}, never a hardcoded figure. The v2.0.0 N=60K-era values (0.1719 / 0.1535) are superseded.

\textbf{Fee-regime + node-count dependence:} the ratio tracks both the fee market and the replication factor. Under the earlier N=60K assumption the average was 0.156--0.172 (dimensionless) with 100\% below $1\times$; at the \textbf{pre-re-base} N=32K it rises to $\sim$0.22--0.29 (\textbf{$\sim$0.27--0.35 at $N=26{,}586$}) with $\sim$98.7--100\% below $1\times$ (a few high-fee blocks exceed coverage in the dated capture). Because $N$ is not directly observable and independent estimates span 10K--100K, the honest headline is the \textbf{$N$-band range: $\sim$0.07--0.71} (inverse-linear in $N$, \S\ref{sec:5.4}). The headline is a \emph{distribution over time and parameters}, not a point.
...
\textbf{Pre-re-base record ($N=32{,}000$).} This section preserves the original derivation, which was written against the then-canonical $N=32{,}000$ (an address-manager sample). The canonical $N$ is now \textbf{26{,}586} (see the re-base note at the top of the paper): multiply every SCCR value in this section by \textbf{1.2036} to place it on the current basis. The \emph{reasoning} --- that $N$ dominates and that the strong ``100\% below $1\times$'' form does not survive a favourable $N$ --- is unchanged.
...
\end{itemize}
\textbf{Live recompute (2026-08-02 baseline, SCCR $=$ 0.2228 at N=32K):} the average now inverts at $N \approx$ 7{,}130 nodes or BTC $\approx$ \$283{,}000; the ``100\% below $1\times$'' break on the dated capture is unchanged at $N \approx$ 49{,}200. Both thresholds are reported; model-spec v2.1.1 retains the v2.0.0-era values (10.3K / \$366K).
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{A better storage-cost model reverses the conclusion.} The denominator ($C \cdot T \cdot N / R_{\mathrm{blocks}}$) rests on a homogeneous node-cost model, an assumed $T=10$ horizon, and the measured reachable-node count (26{,}586; a floor). If a defensible refinement --- measured pruned-vs-archival retention, regional cost heterogeneity, or corrected discounting --- moves the banded result ($\sim$22--29\% coverage, most blocks below $1\times$) to a qualitatively different conclusion, the paper's central claim is falsified in its current form.
