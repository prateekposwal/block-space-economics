...
| 3 | Mempool pressure | **A** | **B*** | `mempool_fee_histogram.json`, `mempool_tx` per snapshot; `block_interval.json`; **historical mempool-count/mempool-size daily series FROZEN 2016-06→2026** (`data/mempool_congestion_series.json`, `tools/research/historical_series_extension.py`): 2016-2026 per-era congestion reads — 2024 was the congestion peak (96.6% of days >50K txs, 72.6% >100K), 2023 Ordinals 58.2% >50K, 2017 27.4% (peak 173K), 2022 empty | Pre-2016-06 congestion (2H2 earlier episodes) remains reconstruction-only — mempool charts start 2016-06; grade B* for the covered era | `data/`, `data/mempool_congestion_series.json`, `captured-data/historical/` |
...
| 9 | Node count / distribution | **B** | **C** | `node_census_series.json` (btcnodes.io snapshot API, 3,981 snapshots 2026-05-08→09-15, ~26.6K reachable nodes, primary source); `node_census.json` addrman sample is an **address count, not a node count** (61,302 as of 2026-09-20; the earlier 32,000 was a truncated request, not a cap); `liveConnections=8` | **2017-12-11 and 2026 are primary-anchored** (N=11,891 and N=26,635 in the SCCR N table, grade B\*); pre-2018 continuous series not recoverable (btcnodes retains ~4mo; wayback has only 2017-12 API capture); 2013-2016 and 2018-2025 remain approximation | `data/node_census_series.json`, `data/node_census_anchors.json`, `tools/research/node_census_capture.py`, `research/node-census-staleness-note.md` |
| 10 | Mining hashrate level | **B** | **A** | `hashrate.json`: 240 points from 2026-08-19 (~934 EH/s); **historical hash-rate TH/s daily series FROZEN 2009→2026** (`captured-data/historical/blockchain.info/hash-rate.json`) + **difficulty FROZEN 2009→2026** (`data/difficulty_series.json`, `tools/research/historical_series_extension.py`), latest 127.45T | Live hashrate.json cadence is currently snapshot-batch (extend to continuous if needed) | `data/hashrate.json`, `data/difficulty_series.json`, `captured-data/historical/` |
...
| Node census protocol re-worked (independent reachable-node series + live inbound) | row 9: C → B — **DONE 2026-09-16** (btcnodes series replaces the addrman address sample; live now B) |
